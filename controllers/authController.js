...
const signToken = (id) => {
  // The secret is what we use to sign the token
...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    // The cookie can not be access or modified in any way by the browser, preventing XSS
    httpOnly: true,
  };

  // Only sent cookies using https and only in production
  if (process.env.NODE_ENV === "production") cookieOptions.secure = true;

  res.cookie("jwt", token, cookieOptions);

  // Don't send the encripted password to the client
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // As password is disabled from selects, we have to use the + token
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // We have to turn validation off, or its gonna require email and password
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // User.findByIdAndUpdate WONT WORK, it wont run the pre functions and validators we defined.
  await user.save();

  // Log user in, sending new JWT
  createSendToken(user, 200, res);
});
